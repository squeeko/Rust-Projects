...
pub const CURRENT_TARGET: Target = {
    ...
    let target = if let Ok(feature) = Feature::new(arch, "sse4.1") { target.with_feature(feature) } else { target };
    let target = if let Ok(feature) = Feature::new(arch, "ssse3") { target.with_feature(feature) } else { target };
    target
};
